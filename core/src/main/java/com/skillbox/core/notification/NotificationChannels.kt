...
object NotificationChannels {
    ...
    fun isNotificationsEnabled(context: Context, notificationManager: NotificationManager): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = notificationManager.getNotificationChannel(EVENT_CHANNEL_ID)
            if(channel == null)
                true
            else
                channel.importance == NotificationManager.IMPORTANCE_HIGH && NotificationManagerCompat.from(context).areNotificationsEnabled()
        } else
